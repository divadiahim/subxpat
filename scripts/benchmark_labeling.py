...
Runs labeling_explicit() in both modes on a configurable set of benchmarks,
...
def main():
    ...
    parser.add_argument(
        ...
        metavar="BENCHMARK",
        help="Space-separated list of benchmark names (without .v extension)",
    )
    parser.add_argument(
        "--partial-cutoff", type=int, default=DEFAULT_PARTIAL_CUTOFF,
        help=f"Error threshold passed to the labeller (default: {DEFAULT_PARTIAL_CUTOFF})",
    )
    parser.add_argument(
    ...
    # Write CSV (overwrite each run so the file stays clean).
